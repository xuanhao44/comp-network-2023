...
.. code-block:: sh
   :emphasize-lines: 1-3,5-12,14-16,18-21,23-25
   :linenos:

      <Quidway>system-view  // 由用户视图进入系统系统
      [Quidway]ping  //测试网络连通性
      [Quidway]reboot //重启交换机/路由器

      [Quidway]display interface  //查看路由器的接口状态
      [Quidway]display ip interface  //查看接口与IP相关的信息
      [Quidway]display ip interface brief  //查看接口状态
      [Quidway]display ip routing-table  //查看路由表
      [Quidway]display vlan  //查看vlan信息
      [Quidway]display port vlan  //查看端口vlan信息
      [Quidway]display current-configuration  //查看运行的配置文件
      [Quidway]display interface Gigabitethernet 0/1 //显示以太网端口g0/0/1 的信息

      [Quidway]interface Gigabitethernet 0/0/1 //进入以太网端口g0/0/1视图
      [Quidway-Gigabitethernet0/0/1]shutdown //关闭以太网端口
      [Quidway-Gigabitethernet0/0/1]undo shutdown //打开以太网端口

      [Quidway-Gigabitethernet0/0/1]port link-type access //设置端口为 Access 端口
      [Quidway-Gigabitethernet0/0/1]port link-type hybrid //设置端口为 Hybrid 端口
      [Quidway-Gigabitethernet0/0/1]port link-type trunk //设置端口为 Trunk 端口
      [Quidway-Gigabitethernet0/0/1]undo port link-type //恢复端口的链路类型为缺省的 Access 端口

      [Quidway-Gigabitethernet0/0/1]quit //从端口视图退出到系统视图
      [Quidway]quit //从系统视图退出到用户视图
      <Quidway>
