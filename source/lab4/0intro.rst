...
基于端口的VLAN
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
基于端口的VLAN是用以太网交换机的物理端口来划分广播域，也就是说，交换机可以通过改变物理端口的VLAN ID，使该物理端口添加到某一个VLAN ID之中，因此当连接交换机的用户发生变化时，也无需更改所对应的物理配线。根据IEEE 802.1Q标准规定，交换机在初始化状态时所有端口默认VLAN ID =1，如果用户对某个端口设置成VLAN ID=2，从而使该端口归属VLAN2。
