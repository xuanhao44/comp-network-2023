...
那有同学可能就问，为什么arp_out()函数中上层协议类型一定是IP协议，不能是其他协议吗？因为本协议栈是基于TCP/IP协议模式，网络层主要通过IP协议来实现数据包的传输（当然还需要众多支持IP的相关辅助技术，如ARP、ICMP、IPsec等等），传输层中的TCP/UDP协议也都是在网络层的IP协议的基础上的。故，本协议栈arp_out()函数中上层协议类型只需关注IP协议即可。
